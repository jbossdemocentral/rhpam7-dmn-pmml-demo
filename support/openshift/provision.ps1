...
Function Create-Application() {
  ...
  Write-Output "Creating Ansible Playbook Bundle pod."
  oc run apb --restart=Never --image="quay.io/ddoyle/rhpam-dmn-pmml-demo-apb:latest" --image-pull-policy=Always -- provision -vvv -e namespace=$($PRJ[0]) -e IMAGE_STREAM_NAMESPACE=$IMAGE_STREAM_NAMESPACE  -e openshift_token=$(oc whoami -t)
